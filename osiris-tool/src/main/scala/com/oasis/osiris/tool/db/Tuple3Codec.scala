...
with VersionAgnostic[(T1, T2, T3)]
{
	...
	def this(eltCodec1: TypeCodec[T1], eltCodec2: TypeCodec[T2], eltCodec3: TypeCodec[T3])
	(implicit protocolVersion: ProtocolVersion, codecRegistry: CodecRegistry)
	{
		this(
	...
	override protected def deserializeAndSetField(input: ByteBuffer, target: (T1, T2, T3), index: Int,
		protocolVersion: ProtocolVersion): (T1, T2, T3) = index match
	{
